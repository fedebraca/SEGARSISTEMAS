...
@section('cont')
    <div class="viewCont" v-cloak>
        <h1 class="ui dividing header">Gestión de Incidentes</h1>
            ...
            <tbody v-show="!cargandoTabla">
                <tr v-for="d in datos">
                    ...
                    <td class="center aligned">
                        <button class="ui button mini icon" :class="{green: d.calc == 1, red: d.calc == 0, loading: cargandoCalc == $index}" v-on:click="calc($index,d.id)">
                            <i class="icon" :class="{checkmark: d.calc == 1, remove: d.calc == 0}"></i>
                        </button>
                    </td>
                    <td class="center aligned" v-if="cargandoActivo != $index">
                    ...
                    <td class="center aligned" v-if="cargandoActivo == $index"><i class="spinner loading icon"></i></td>
                    <td><% d.created_at | fecha %></td>
                    <td><% d.updated_at | fecha %></td>
                    <td>
            ...
            <div class="content">
                <div class="ui form editar">
                    <div class="field">
                        ...
                        <input type="text" name="desig" v-model="e.desig">
                    </div>
                </div>
            </div>
            <div class="actions">
        ...
        <div class="ui small modal agregar">
            ...
            <div class="content">
                <div class="ui form agregar">
                    <div class="field">
                        <label>Descripción</label>
                        <input type="text" name="desig" v-model="d.desig">
                    </div>
                </div>
            </div>
            <div class="actions">
...
@section('script')
    <script>
        $(document).ready(function () {
            var urlAgregar = '{{ URL::to('incidente/agregar') }}';
            var urlEdit = '{{ URL::to('incidente/editar') }}';
            var urlElim = '{{ URL::to('incidente/eliminar') }}';
            var urlActivar = '{{ URL::to('incidente/activar') }}';
            var urlCalc = '{{ URL::to('incidente/activa-calc') }}';
            var urlTabla = '{{ URL::to('incidente/tabla') }}';
            ...
            var viewCont = new Vue({
                ...
                methods: {
                    calc: function ($index, id) {
                        var self = this;
                        self.cargandoCalc = $index;
                        var calc = (self.datos[$index].calc == 0) ? 1 : 0;
                        $.post(urlCalc, {'_token': token, 'id': id, calc: calc})
                                .then(function (r) {
                                    if (r.result == 'success') {
                                        self.datos[$index].calc = calc;
                                    }
                                    self.cargandoCalc = -1;
                                }, function () {
                                    self.cargandoCalc = -1;
